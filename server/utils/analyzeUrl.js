...
// Save cache every 10 minutes (600000 ms)
setInterval(() => {
...
async function getDomainAgeDays(domain) {
  try {
    ...
    const creationDate = data.creationDate || data.createdDate || data.created || data.registered;
...
async function analyzeUrl(url) {
  ...
  try {
    ...
    // 1️⃣ Google Safe Browsing API check
    ...
    // 2️⃣ OpenPhish check (cached set)
    const openPhishSet = getOpenPhishSet();
    const isUnsafeOpenPhish = [...openPhishSet].some(phishUrl => phishUrl && url.includes(phishUrl));

    // 3️⃣ WHOIS domain age check
    const domainAgeDays = await getDomainAgeDays(hostname);
    const isNewDomain = domainAgeDays !== null && domainAgeDays < 30;

    // 4️⃣ DNS lookup to get IP
    ...
    // 6️⃣ Custom keyword-based heuristic
    const suspiciousKeywords = ["login", "verify", "update", "secure", "bank", "account", "free", "gift", "win"];
    const hasSuspiciousKeyword = suspiciousKeywords.some(keyword =>
      hostname.toLowerCase().includes(keyword)
    );

    // 7️⃣ Suspicious TLD check
    const tld = hostname.split('.').pop().toLowerCase();
    const suspiciousTlds = ["xyz", "top", "club", "live", "win", "work", "gq", "tk"];
    const hasSuspiciousTld = suspiciousTlds.includes(tld);

    // 7 HTTPS check
     const isHttps = protocol === "https:" || url.startsWith("https://");

    // 8) SSL certificate (only attempt for HTTPS)
    let ssl = null;
    ...
    let sslExpiringSoon = false;
     ...
     if (isHttps) {
      ...
      if (ssl) {
        ...
        sslExpiringSoon = typeof ssl.daysRemaining === 'number' && ssl.daysRemaining <= 14;
      }
    }
    // 9️⃣ Final scam decision
    const isScamFinal =
      ...
      // Optional: a little scoring polish (0=bad, 100=good)
      let score = 100;
